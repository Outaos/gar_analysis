"""
----------------------------------------------------------------------------------------------------------------
    PYTHON SCRIPT: gar_8232.py

    Author:       BCTS TOC - Graydon Shevchenko
    Purpose:      Class file containing methods specific to GAR 8-232 - Grizzly Bear
...
class Gar8232:
    """
    Class:
        class object for GAR 8-232
    """
    def __init__(self, gar, output_xls, logger, gar_config):
        """
        Function:
            Initializes the Gar8232 object and its attributes
        Args:
        ...
        self.lst_footers = [r'* Suitable Cover Target is 10% of the total cell area',
    ...
    def calculate_level(self, bec=None, age=None, spp=None, cc=None, slp=None, thlb=None, diam=None, pct=None, gfa=None,
        ...
        Args:
            ...
            target (float): target
            height (float): tee height

        Returns:
            str: level assigned to record
        """
        level = None
        if height == '>= 19.5m' and shp_area >= 5:
    ...
    def calculate_targets(self):
        ...
        for op_area in self.dict_total_area:
            for pcell in self.dict_total_area[op_area].pcell:
                ...
                self.dict_total_area[op_area].pcell[pcell].target = cell_area * 0.1

        for pcell in self.dict_cell_area:
            cell_area = self.dict_cell_area[pcell].hectares
            self.dict_cell_area[pcell].target = cell_area * 0.1
