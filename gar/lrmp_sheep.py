"""
----------------------------------------------------------------------------------------------------------------
    PYTHON SCRIPT: lrmp_sheep.py

    Author:       BCTS TOC - Graydon Shevchenko
    Purpose:      Class file containing methods specific to LRMP Bighorn Sheep and Derenzy Sheep
...
class LrmpSheep:
    """
    Class:
        class object for LRMP Sheep
    """
    def __init__(self, gar, output_xls, logger, gar_config):
        """
        Function:
            Initializes the LrmpSheep object and its attributes
        Args:
        ...
        self.str_ch = 'CH'
        self.str_nh = 'NH'
        self.str_suitable = 'Sheep Habitat'
        self.str_no_harv = 'No Harvest'

        self.str_imm = 'Immature'
        self.lst_level = [self.str_suitable]
        self.lst_headers = ['Planning Cell', 'Sheep Habitat Target (ha) *', 'Analysis Area', 'Sheep Habitat (ha)',
                            '+/- (ha) *', 'Total Area (ha) **']

        self.lst_footers = [r'* Sheep Habitat Target is 10% of the total cell area',
                            r'** Total Area is the area of the cell where Private Land, woodlots, '
                            r'and crown federal land has been removed.'
                            ]

    def calculate_level(self, bec=None, age=None, spp=None, cc=None, slp=None, thlb=None, diam=None, pct=None, gfa=None,
                        notes=None, op_area=None, pcell=None, shp_area=None, target=None, height=None):
        """
        Function:
        ...
        level = None
        if height and cc:
            if gfa == 'Y' and height >= 16 and cc >= 26:
                level = self.str_suitable
    ...
    def write_excel(self):
        ...
        for op_area in [o for o in sorted(self.dict_total_area)]:
            ...
            for c in self.lst_headers:
                ...
                if c.startswith(('Total', 'Sheep Habitat (ha)')):
